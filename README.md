...
Enter the development shell. This installs the Git hook set generated by
`git-hooks.nix` and exposes the Ruby toolchain built from `gemset.nix`:
...
Amgi uses the [`ruby-nix`](https://github.com/inscapist/ruby-nix) workflow to
manage the Ruby dependency graph through `gemset.nix`, and exposes those gems
in the development shell through Nix's `bundlerEnv`.

The update flow is:

1. change `Gemfile`
2. refresh `Gemfile.lock` with `nix develop -c bundle-lock` or
   `nix develop -c bundle-update`
3. regenerate `gemset.nix` with `nix develop -c bundix`

That keeps the Nix shell, RuboCop, RSpec, and Git hooks pinned to the same gem
set while preserving runnable gem executables in the shell.

### Git Hooks

The repository checks are wired through
[`git-hooks.nix`](https://github.com/cachix/git-hooks.nix).

Current automated hooks run:

- YAML syntax validation
- RuboCop lint
- RSpec test suite

`bin/check` delegates to that same hook set inside `nix develop`, so manual and
automatic validation use the same pipeline.
